...
local Config = {
    ...
    box_size = 280,
    speed_ms = 40,
    offset_left = -3 -- DÉCALAGE DE 3 PIXELS GAUCHE
}
...
statusText.setText("1. CLIQUE SUR ACTIVER VISION")
...
btnRecord.setOnClickListener(function()
    local ok = false
    -- Essayer de demander la permission proprement
    pcall(function()
    ...
    if ok then
        ...
        setInfo("✅ VISION OK ! LANCE L'AIM", Color.GREEN)
        btnRecord.setBackgroundColor(0xFF2E7D32)
    else
        setInfo("❌ ERREUR CAPTURE (VOIR RÉGLAGES)", Color.RED)
        print("AIDE : Accepte la fenêtre 'Commencer maintenant' qui s'affiche")
    end
end)
...
lpBox.gravity = 17 
lpBox.x = Config.offset_left -- ICI LE DÉCALAGE DE -3 PIXELS VERS LA GAUCHE

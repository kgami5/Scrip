...
local function createTarget(col, txt, x, y, cb)
    local v = TextView(activity); v.setText(txt); v.setGravity(17); v.setTextSize(8); v.setTextColor(-1)
    local gd = GradientDrawable(); gd.setColor(col); gd.setShape(1); gd.setStroke(1,-1); v.setBackground(gd)
    local lp = WindowManager.LayoutParams(90,90,2038,8,-3); lp.gravity=51; lp.x=x; lp.y=y
    local tx,ty
...
-- BOUTON ZEN (Petit)
local btnZen = Button(activity); btnZen.setText("Z"); btnZen.setTextColor(Color.CYAN)
...
local lpZen = WindowManager.LayoutParams(100,100,2038,8,-3); lpZen.gravity=51; lpZen.x=50; lpZen.y=150
...
-- SCROLL (Hauteur forcée petite)
local scroll = ScrollView(activity)
-- HAUTEUR FIXE : 400 pixels (Tient sur n'importe quel écran)
...
-- Header Mini
local t = TextView(activity); t.setText("ZEN MINI V12"); t.setTextColor(Color.MAGENTA); t.setTextSize(12); t.setGravity(17)
content.addView(t)

-- Fonction Switch Mini
local function addS(txt,k)
    local h = LinearLayout(activity); h.setOrientation(0); h.setGravity(16)
    local tv = TextView(activity); tv.setText(txt); tv.setTextColor(-1); tv.setTextSize(10); tv.setLayoutParams(LinearLayout.LayoutParams(0,-2,1.0))
    local s = Switch(activity); s.setScaleX(0.8); s.setScaleY(0.8); -- Réduit la taille du switch
...
-- Fonction Slider Mini
local function addSl(txt,k,max)
    local tv = TextView(activity); tv.setText(txt..": "..config[k]); tv.setTextColor(Color.LTGRAY); tv.setTextSize(10); content.addView(tv)
    local sk = SeekBar(activity); sk.setMax(max); sk.setProgress(config[k])
    sk.setLayoutParams(LinearLayout.LayoutParams(-1, 50)) -- Slider moins haut
...
local btnHide = Button(activity); btnHide.setText("Cacher Cibles"); btnHide.setTextSize(10); btnHide.setHeight(80)
...
local btnClose = Button(activity); btnClose.setText("Fermer"); btnClose.setTextSize(10); btnClose.setTextColor(Color.RED); btnClose.setHeight(80)
btnClose.setOnClickListener(function() menu.setVisibility(8) end); content.addView(btnClose)

-- Fenêtre Menu Plus Etroite
local lpMenu = WindowManager.LayoutParams(450, WindowManager.LayoutParams.WRAP_CONTENT, 2038, 8, -3)

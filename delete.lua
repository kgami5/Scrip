...
local Config = {
    ...
    scan_range = 60,     -- Rayon de recherche du rouge (Zone de Lock)
    scan_step = 5,       -- Précision (plus bas = plus précis mais plus lent)
    sensitivity = 0.8,   -- Vitesse du Lock (Aim assist)
    speed_ms = 10        -- Vitesse de la boucle (Ultra rapide)
}
...
-- ================= 5. LOGIQUE AIMLOCK & TIR (C'EST ICI QUE CA SE PASSE) =================

-- Cette fonction tire ET corrige la visée (Lock)
function aimAndShoot(targetX, targetY)
    if not service then return end -- "service" est la variable globale d'accessibilité

    local path = Path()
    path.moveTo(CX, CY)
    
    -- Calcul du Lock : on glisse légèrement vers le rouge détecté
    ...
    -- GESTE : Un appui très court (10ms) qui fait office de tir et de micro-glissement
...
scanLoop = Runnable({ run = function()
    ...
    if img then
        ...
        -- RECHERCHE DU ROUGE DANS LA ZONE CENTRALE (Pour le Lock)
        local totalX, totalY, count = 0, 0, 0
        local range = Config.scan_range
        
        -- On scanne un carré autour du viseur
        ...
        -- SI ON A TROUVÉ DES PIXELS ROUGES
        if count > 0 then
            -- On calcule le centre de la masse rouge (l'ennemi)
            ...
            -- ACTION : TIR + LOCK vers le centre du rouge
            aimAndShoot(targetX, targetY)
        else
            -- VISUEL : Carré ROUGE (Rien détecté)
            local gd = GradientDrawable()
            gd.setStroke(4, Color.RED)
            BoxWindow.setBackgroundDrawable(gd)
        end
        
        bitmap.recycle() -- Important pour la mémoire
    end

...
-- Tentative d'importation du module images si disponible
pcall(function() import "com.stardust.autojs.project.Images" end)
...
statusLayout.setPadding(25, 10, 25, 10)

local statusText = TextView(activity)
statusText.setText("ÉTAT: EN ATTENTE")
...
-- ================= LOGIQUE DE CAPTURE (FIX CRASH) =================

function startCapturePermission()
    -- On teste plusieurs méthodes pour éviter l'erreur "nil"
    local success = false
    try {
        run = function()
            -- Méthode 1: images.requestScreenCapture
            if images and images.requestScreenCapture then
                success = images.requestScreenCapture(false)
            -- Méthode 2: requestScreenCapture global
            elseif requestScreenCapture then
                success = requestScreenCapture(false)
            end
        end,
        error = function(e)
            print("Erreur de capture: "..tostring(e))
        end
    }
    return success
end
...
function getScreenShot()
    if images and images.captureScreen then
...
function getTarget()
    ...
    if count > 0 then
        updateStatus("🎯 CIBLE TROUVÉE", Color.GREEN)
        return { x = sumX / count, y = sumY / count }
    end
    updateStatus("🔍 RECHERCHE ROUGE...", Color.YELLOW)
    return nil
end
...
mainView.setBackgroundDrawable(GradientDrawable().setColor(0xF01A1A1A).setStroke(4, Color.CYAN).setCornerRadius(20))
...
btnRecord.setOnClickListener(function()
    threads.start(function()
        if startCapturePermission() then
            Config.is_recording = true
            updateStatus("✅ VISION OK", Color.GREEN)
            activity.runOnUiThread(Runnable({run=function() 
                btnRecord.setBackgroundColor(0xFF2E7D32)
            end}))
        else
            updateStatus("❌ ÉCHEC VISION (Vérifier Autorisations)", Color.RED)
        end
    end)
end)
...
-- LE CARRÉ (DÉCALÉ DE 3 PIXELS À GAUCHE)
...
print("✅ Recodage terminé : Décalage gauche -3px appliqué.")
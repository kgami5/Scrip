...
local Config = {
    ...
    box_size = 280,
    speed_ms = 40,
    offset_x = -3 -- DÉCALAGE 3 PIXELS GAUCHE APPLIQUÉ ICI
}
...
statusText.setText("ÉTAPE 1 : CLIQUE SUR VISION")
...
function getTarget()
    ...
    local step = 15
    local startX = (CX - (Config.box_size / 2)) + Config.offset_x -- Scan aussi décalé
...
btnRecord.setOnClickListener(function()
    -- ASTUCE : On cache le menu pour forcer Android à montrer la fenêtre de permission
    mainView.setVisibility(8)
    setInfo("ACCEPTE LA FENÊTRE QUI VA APPARAÎTRE", Color.CYAN)
    ...
    handler.postDelayed(Runnable({run=function()
        ...
        mainView.setVisibility(0) -- Le menu revient
    end}), 500)
end)
...
-- LE CARRÉ DE VISÉE (DÉCALÉ À GAUCHE DE 3px)
...
lpBox.gravity = 17 
lpBox.x = Config.offset_x -- DECALAGE GAUCHE -3px APPLIQUÉ

...
local Config = {
    active = false,
    is_recording = false,
    box_size = 80,      -- Taille du scan autour du viseur
    speed_ms = 25,      -- Rapidité de la boucle
    offset_x = -52,    -- TON DÉCALAGE (Viseur à gauche)
    magnet_power = 0.8  -- Force du suivi
}
...
-- Centre corrigé (Milieu de l'écran + ton décalage)
...
statusText.setText("STATUT : EN ATTENTE")
...
function getTarget()
    -- Utilisation directe du module global 'images' fourni par l'app
    ...
    for y = 0, Config.box_size, step do
        for x = 0, Config.box_size, step do
            ...
            -- DÉTECTION DU ROUGE VIF (Viseur CODM)
            if r > 210 and g < 65 and b < 65 then
                sumX = sumX + (scanX + x)
                sumY = sumY + (scanY + y)
                count = count + 1
            end
...
mainLoop = Runnable({ run = function()
    ...
    if target then
        ...
        setInfo("🔒 LOCK : CIBLE DÉTECTÉE", Color.RED)
...
mainView.setBackgroundDrawable(CreateShape(0xF0121212, Color.CYAN))
mainView.setPadding(40, 40, 40, 40)
mainView.setVisibility(8)

btnRecord = Button(activity)
btnRecord.setText("1. INITIALISER VISION")
...
btnScan.setText("2. DÉMARRER LOCK")
...
-- Gérer la réponse de la capture (onActivityResult)
function onActivityResult(requestCode, resultCode, data)
    if requestCode == 1 and resultCode == -1 then
        if images and images.requestScreenCapture(false) then

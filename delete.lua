...
local Config = {
    ...
    box_size = 280,
    speed_ms = 40,
    offset_left = -3 -- DÉCALAGE -3px GAUCHE DEMANDÉ
}
...
-- Type de fenêtre pour Android récent (Oppo/Xiaomi friendly)
local OVERLAY_TYPE = 2038 

-- ================= VÉRIFICATION PERMISSION =================
if not Settings.canDrawOverlays(activity) then
    print("⚠️ AUTORISE LE DESSIN PAR-DESSUS LES APPS !")
...
-- Sécurité pour éviter le crash au lancement
...
function getTarget()
    ...
    setInfo("🔍 RECHERCHE ROUGE...", Color.YELLOW)
    return nil
end
...
lpBox.gravity = 17 
lpBox.x = Config.offset_left -- DECALAGE GAUCHE -3px

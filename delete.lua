...
-- Supprime les anciennes vues pour éviter les bugs au lancement
pcall(function() wm.removeView(statusLayout) end)
pcall(function() wm.removeView(menu) end)
pcall(function() wm.removeView(box) end)

-- Configuration
local Config = {
    active = false,
    is_recording = false,
    offset_x = -52, -- TON DÉCALAGE GAUCHE
    box_size = 150,   -- Zone resserrée sur le viseur
    speed = 25       -- ms
}
...
-- Fonction de dessin universelle
...
statusText.setText("1. ACTIVE LA VISION")
...
function scanTarget()
    ...
    -- Scan ultra-rapide du centre
    for y = 0, Config.box_size, 5 do
        for x = 0, Config.box_size, 5 do
            local px = images.getPixel(img, scanX + x, scanY + y)
            local r = (px >> 16) & 0xFF
            local g = (px >> 8) & 0xFF
            local b = px & 0xFF

            -- Détection du rouge vif du viseur CODM
...
mainLoop = Runnable({run=function()
    ...
    if target then
        setTxt("🎯 LOCK : ROUGE DÉTECTÉ", Color.RED)
        ...
        if s then
            ...
            b.addStroke(luajava.bindClass("android.accessibilityservice.GestureDescription$StrokeDescription")(p, 0, 40))
...
menu.setBackgroundDrawable(CreateGD(0xF0101010, Color.YELLOW))
...
btn1.setOnClickListener(function()
    setTxt("PATIENTE... CLIQUE SUR 'COMMENCER'", Color.CYAN)
    -- On tente la capture. Si ça erreur, on réessaie après un court délai
    local function tryCapture()
        if pcall(function() images.requestScreenCapture(false) end) then
            ...
            btn1.setText("VISION : OK")
            setTxt("VISION ACTIVE ✅", Color.GREEN)
        else
            setTxt("❌ ERREUR : RÉESSAYE", Color.RED)
        end
    end
    tryCapture()
end)
...
btn2.setOnClickListener(function()
    if not Config.is_recording then 
        setTxt("⚠️ ÉTAPE 1 D'ABORD", Color.YELLOW)
        return 
    end
    Config.active = not Config.active
    btn2.setBackgroundColor(Config.active and 0xFF2E7D32 or 0xFFC62828)
    btn2.setText(Config.active and "STOP AIM" or "2. START AIM")

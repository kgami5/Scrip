...
local function CreateShape(color, strokeColor, strokeWidth)
    ...
    if strokeColor then
        gd.setStroke(strokeWidth or 4, strokeColor)
    end
    return gd
end

-- ================= GESTION CAPTURE (PROJECTION) =================
...
function getTarget()
    -- Capture d'écran via le moteur de l'app
    local img = nil
...
boxView.setBackgroundDrawable(CreateShape(0, Color.RED, 5))
